...
public class ClassGroupService {
    ...
    @Autowired
    public ClassGroupService(ClassGroupRepository classGroupRepository, PaymentRepository paymentRepository) {
    ...
    public ClassGroup getGroupById(Long id) {
        return classGroupRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Group not found with ID: " + id));
    }
    ...
    public List<GroupWithStudentsDto> getAllGroupsWithStudents() {
        ...
        for (Object[] result : results) {
            ...
            if (debt == null) {
                debt = 0.0;  // Ensure that debt is set to 0 if no payments exist
            }
